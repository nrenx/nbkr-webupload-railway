...
# Make selenium optional (legacy support)
try:
    ...
    WEBDRIVER_MANAGER_AVAILABLE = False
    print("Warning: selenium is not installed. Legacy browser automation will be disabled.")
...
class PersonalDetailsScraper:
    ...
    def initialize_playwright(self):
        ...
        try:
            logger.info(f"Initializing personal details scraper in {'headless' if self.headless else 'visible'} mode")
    ...
    def authenticate(self, retry_count=0) -> bool:
        ...
        if self.page:
            try:
                logger.info(f"Authenticating using Playwright (attempt {retry_count + 1}/{min(2, self.max_retries) + 1})...")
                # Go directly to the personal details URL which will redirect to login page
                self.page.goto(PERSONAL_DETAILS_URL)
                ...
                if "login" not in current_url.lower():
                    ...
                    if "invalid" in page_content.lower() or "incorrect" in page_content.lower() or "failed" in page_content.lower():
    ...
    def navigate_to_personal_details_page(self) -> Optional[BeautifulSoup]:
        ...
        try:
            ...
            if self.page:
                logger.info(f"Navigating to personal details page using Playwright: {PERSONAL_DETAILS_URL}")
                self.page.goto(PERSONAL_DETAILS_URL)
                ...
                current_url = self.page.url
                if "selectionForRollNos.php" in current_url:
                    logger.info("Successfully navigated to personal details page using Playwright")
    ...
    def select_form_filters(self, academic_year: str, year_of_study: str, branch: str, section: str) -> Optional[BeautifulSoup]:
        ...
        try:
            # Use Playwright if available
            if self.page:
                logger.info(f"Selecting form filters using Playwright: {academic_year}, {year_of_study}, {branch}, {section}")

                # Select academic year
                academic_year_select = self.page.wait_for_selector('select[name="acadYear"]', state='visible')
                academic_year_select.select_option(label=academic_year)
                self.page.wait_for_timeout(500)  # Wait for the page to update

                # Select year of study
                year_select = self.page.wait_for_selector('select[name="yearSem"]', state='visible')
                ...
                if year_of_study in year_sem_mapping:
                    year_select.select_option(value=year_sem_mapping[year_of_study])
                else:
                    year_select.select_option(label=year_of_study)
                ...
                if branch in branch_mapping:
                    branch_select.select_option(value=branch_mapping[branch])
                else:
                ...
                # Get all options
                options = section_select.query_selector_all('option')
                ...
                for option in options:
                    ...
                    if section == option_text or section == option_value:
                        section_select.select_option(value=option_value)
                        option_found = True
                ...
                if not option_found:
                    ...
                    section_select.select_option(index=0)
                    selected_option = section_select.evaluate('el => el.options[el.selectedIndex].text')
                    logger.warning(f"Could not find section '{section}', selected first option: {selected_option}")

                self.page.wait_for_timeout(1000)  # Increased wait time for the page to update
                ...
                try:
                    ...
                    all_checkboxes = self.page.query_selector_all("input[type='checkbox']")
                    ...
                    for checkbox in all_checkboxes:
                        try:
                            ...
                            if not is_checked:
                                ...
                                self.page.wait_for_timeout(200)  # Wait for scroll

                                # Click the checkbox
                                checkbox.click()
                                self.page.wait_for_timeout(300)  # Increased delay between clicks
                                selected_count += 1
                        except Exception as e:
                ...
                try:
                    submit_button = self.page.query_selector("input[type='submit'][value='Get List of RollNos']")
                    if submit_button:
                ...
                if not submit_button:
                    try:
                        ...
                        if submit_buttons and len(submit_buttons) > 0:
                            ...
                            logger.info(f"Found submit button with value: {button_value}")
                    except Exception as e:
                ...
                try:
                    ...
                    try:
                        self.page.wait_for_selector('table', timeout=self.timeout * 1000)  # Convert to milliseconds
                    except PlaywrightTimeoutError:
                        # If we time out waiting for tables, check if the page has any content
                        logger.warning("Timed out waiting for tables, checking if page has content")
                        # Continue anyway, as the page might have loaded but tables might be structured differently

                    # Then wait a bit more for the data to fully load
                    self.page.wait_for_load_state('networkidle')
                    self.page.wait_for_timeout(3000)  # Increased wait time after form submission
                    ...
                    if tables and len(tables) > 0:
                        ...
                        if has_data:
                            logger.info("Form submitted successfully using Playwright - found tables with data")
                            return result_soup
                        else:
